...
class SwitchableCycleGANModel(BaseModel):
    def name(self):
        return 'SwitchableCycleGANModel'
    ...
    def set_input_test(self, input, latent,dataset_name):
        ...
        if dataset_name == 'ixi':
            if self.task == 't1_t2':
                ...
                self.task_info = torch.tensor([[1.0, 0.0]], requires_grad=False ).cuda(self.gpu_ids[0])
            else:
                #T2->PD
                self.input_A = torch.unsqueeze(input_A[:,1],axis=1)
                self.input_B = input_B #torch.unsqueeze(input_A[:,1],axis=1)
                self.task_info = torch.tensor([[0.0, 1.0]], requires_grad=False ).cuda(self.gpu_ids[0])
