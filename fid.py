...
def get_activations(images, batch_size,gpu_ids):
    """
    ...
    inception_network = to_cuda(inception_network,gpu_ids)
...
def calculate_activation_statistics(images, batch_size,gpu_ids):
    ...
    """
    act = get_activations(images, batch_size,gpu_ids)
...
def calculate_fid(images1, images2, use_multiprocessing, batch_size,gpu_ids):
    ...
    mu1, sigma1 = calculate_activation_statistics(images1, batch_size,gpu_ids)
    mu2, sigma2 = calculate_activation_statistics(images2, batch_size,gpu_ids)
    fid = calculate_frechet_distance(mu1, sigma1, mu2, sigma2)
    return fid


def load_images_real(path):
    ...
    for ext in image_extensions:
        print("Looking for images in", os.path.join(path, "*_real_B.{}".format(ext)))
        for impath in glob.glob(os.path.join(path, "*_real_B.{}".format(ext))):
...
def load_images_fake(path):
    ...
    image_extensions = ["png"]
    for ext in image_extensions:
        print("Looking for images in", os.path.join(path, "*_fake_B.{}".format(ext)))
        for impath in glob.glob(os.path.join(path, "*_fake_B.{}".format(ext))):
            image_paths.append(impath)
    first_image = cv2.imread(image_paths[0])
    W, H = first_image.shape[:2]
    image_paths.sort()
...
# Example usage:
# python3 /auto/data2/odalmaz/fid.py --p1 /auto/data2/odalmaz/FedSynth/3_heteregeneous/results/T1_T2_pFLSynth_no_task_info_3_heteregeneous_ablation/test_latest_BRATS_flair_t2/images/ --gpu_ids 1
if __name__ == "__main__":
    from optparse import OptionParser
    parser = OptionParser()
    parser.add_option("--p1", "--path1", dest="path1", 
                      help="Path to directory containing images")
    parser.add_option("--gpu_ids", "--gpu_ids", dest="gpu_ids",
                      help="gpu_ids")
    ...
    path1= options.path1
    path2 = options.path1
    gpu_ids = int(options.gpu_ids)
    batch_size = 1
    images1 = load_images_real(path1)
    print(images1.shape)
    print(images2.shape)
    fid_value = calculate_fid(images1, images2, options.use_multiprocessing, batch_size,gpu_ids)

    print("FID :" + str( '{0:.4g}'.format(fid_value)))