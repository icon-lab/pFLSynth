...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for key in IXI_netG_SD:
            # print(key)
            if 'norm' not in key and 'bn' not in key:# and 'running_mean' not in key and 'running_var' not in key and 'num_batches_tracked' not in key:
                # print(key)
                temp = ixi_coef * IXI_netG_SD[key] + brats_coef * BRATS_netG_SD[key] + midas_coef * MIDAS_netG_SD[key] + fastmri_coef * fastMRI_netG_SD[key]
                IXI_netG_SD[key] = temp
                BRATS_netG_SD[key] = temp
                MIDAS_netG_SD[key] = temp
                fastMRI_netG_SD[key] = temp
